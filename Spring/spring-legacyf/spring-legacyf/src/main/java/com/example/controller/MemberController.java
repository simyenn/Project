...
//자바 빈끼리 연결이 가능하다 (객체가 된다 = 자바빈이 된다)
//스프링이 애노테이션을 통해 객체를 만들어줌
@Controller
...
public class MemberController {
	...
	public String join() {
		...
		return "member/join"; // jsp이름 join.jsp
	}
	...
	public ResponseEntity<String> join(MemberVO memberVo) {
		...
		// 서버에서 데이터를 추가,수정,삭제 후 화면응답을 바로 줄때
...
//      }
	...
	// *************************비밀번호 변경**********************************
	@GetMapping("/passwd3") // /member/passwd
	public String passwd(String id, String passwd) {

		System.out.println("passwd 호출됨...");
		return "member/passwd3"; // jsp이름 passwd.jsp
	}
	
	@GetMapping("/passwd4") // /member/passwd
	public String passwd3(String id, String passwd) {

		System.out.println("passwd 호출됨...");
		return "member/passwd4"; // jsp이름 passwd.jsp
	}
	
	
	
	@PostMapping("/passwd3") // /member/passwd
	public ResponseEntity<String> passwd2 (String id, String passwd) {
		...
		// ID 체크 - 해당 아이디 값이 있는지
		if (memberVO1 != null) {
			isPasswdSame = BCrypt.checkpw(passwd, memberVO1.getPasswd());
		...
		// 로그인 실패(없는 ID or PW가 틀렸을 경우)
		if (memberVO1 == null || !isPasswdSame) {
			HttpHeaders headers = new HttpHeaders();
			headers.add("Content-Type", "text/html; charset=UTF-8");
		...
		// Script에 alert가 포함되어있음
		String str = Script.href("비밀번호 확인 성공!", "/member/passwd4");
...
//
	@PostMapping("/passwd4") // /member/passwd
...
//	*********************************************************************************************
	...
	public String remove() {
		...
		return "member/remove"; // jsp이름 remove.jsp
	}

	@PostMapping("/remove")
	public ResponseEntity<String> remove(String id, String passwd, String rememberMe, HttpSession session,
		...
		if (memberVO != null) {
			...
			if (isPasswdSame == false) { // 비밀번호 일치하지 않음
				...
				headers.add("Content-Type", "text/html; charset=UTF-8");
				message = "비밀번호가 일치하지 않습니다.";
				String str = Script.href(message, "/member/remove");
				return new ResponseEntity<String>(str, headers, HttpStatus.FOUND);
			}
		} else { // memberVO == null // 일치하는 아이디가 없음
			message = "존재하지 않는 아이디 입니다.";
		}
		System.out.println("remove 포스트매핑..");
		session.setAttribute("id", id);
		HttpHeaders headers = new HttpHeaders();
		...
		// Script에 alert가 포함되어있음
		String str = Script.href(message, "/");
